...
and GenUnionCaseRef (amap: ImportMap) m tyenv i (fspecs: RecdField[]) offsetInfo =
    let g = amap.g

    let mkUnionCaseField amap m tyenv i j (fspec: RecdField) offset =
        let ilFieldDef = IL.mkILInstanceField(fspec.Name, GenType amap m tyenv fspec.FormalType, None, ILMemberAccess.Public, offset)
        // These properties on the "field" of an alternative end up going on a property generated by cu_erase.fs
        IlxUnionCaseField (ilFieldDef.With(customAttrs = mkILCustomAttrs [(mkCompilationMappingAttrWithVariantNumAndSeqNum g (int SourceConstructFlags.Field) i j )]))
    ...
    | Some maxReferenceTypeFieldsInAlt ->
        ...
        Array.append
            (referenceTypeFields |> Array.mapi (fun j fspec -> mkUnionCaseField amap m tyenv i j fspec (Some (referenceOffsetStart + referenceWidth * j))))
            (valueTypeFields |> Array.mapi (fun j fspec -> mkUnionCaseField amap m tyenv i j fspec (Some (valueTypeOffsetStart + referenceWidth * j)))) // todo use previous value type field's byte width
    | _ ->
        fspecs |> Array.mapi (fun j fspec -> mkUnionCaseField amap m tyenv i j fspec None)
              ...
              let alternatives =
                  tycon.UnionCasesArray |> Array.mapi (fun i cspec ->
                      { altName = cspec.CompiledName
                        ...
                        altFields = GenUnionCaseRef amap m tyenvinner i cspec.RecdFieldsArray None })
            ...
            let ilFieldDefs =
                mkILFields
                    [ for (_, fldName, fldTy) in flds ->
                        let fdef = mkILInstanceField (fldName, fldTy, None, ILMemberAccess.Private, None)
        ...
        let ilFieldDefs =
             [ for (useGenuineField, ilFieldName, isFSharpMutable, isStatic, _, ilPropType, isPropHidden, fspec) in fieldSummaries do
               ...
               if requiresExtraField then
                   yield mkILInstanceField("__dummy", g.ilg.typ_Int32, None, ILMemberAccess.Assembly, None) ]
           ...
           | TUnionRepr _ ->
               let layout, offsetInfo =
                   if isStructTy g thisTy then
                       ...
                       // todo isn't this always true since isStructTy is true?
                           ...
                           // Structs with no instance fields get size 1, pack 0
                           ILTypeDefLayout.Sequential { Size=Some 1; Pack=Some 0us }, None
                       else
                           ILTypeDefLayout.Sequential { Size=None; Pack=None }, None
                   else
               ...
               let alternatives =
                   tycon.UnionCasesArray |> Array.mapi (fun i ucspec ->
                       { altName = ucspec.CompiledName
                         altFields = GenUnionCaseRef cenv.amap m eenvinner.tyenv i ucspec.RecdFieldsArray offsetInfo
               ...
               let cuinfo =
                  { cudReprAccess = reprAccess
                    ...
                    cudWhere = None
                    cudExplicitStructLayout = offsetInfo.IsSome }
    ...
    | TExnFresh _ ->
        ...
        let ilMethodDefsForProperties, ilFieldDefs, ilPropertyDefs, fieldNamesAndTypes =
            [ for i, fld in Seq.indexed fspecs do
               ...
               let ilFieldDef = IL.mkILInstanceField(ilFieldName, ilPropType, None, ILMemberAccess.Assembly, None)
               let ilPropDef =
                   ILPropertyDef(name = ilPropName,
                                 attributes = PropertyAttributes.None,
                                 setMethod = None,
